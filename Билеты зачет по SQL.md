...
> [!question]- 2. Основные типы данных в языке SQL. Базовые операции над типами.
>
> ***Типы данных***
>
...
> [!question]- 3. Команда SELECT. Назначение и синтаксис. Привести примеры.
>**Назначение**
...
>[!question]- 4. Псевдонимы при выборке данных.Синтаксис и мотивация применения. Привести примеры. Синтаксис и мотивация применения
...
> [!question]- 5. Основные аггрегирующие функции. Привести примеры запросов.
>**Функции**
...
> [!question]- 6. Фильтрация результатов запроса. Привести несколько разных по сложности примеров.
>
>**Определение**
>
...
> [!question]- 7. Группировка данных по признакам. Группировка с фильтрацией. Привести несколько разных по сложности примеров.
>
>**Определение**
>
...
> [!question]- 8. Вложенные запросы. Мотивация использования. Привести несколько разных по сложности примеров.
>
>**Определение**
>
...
> [!question]- 9. JOIN. Различные виды JOIN’ов. Привести несколько примеров с разными видами.
>
>**Определение**
>
...
> [!question]- 10. Различия между JOIN и вложенными запросами (производительность, читабельность, применимость). Привести пример аналогичных запросов с использованием JOIN и подзапросов.
>
...
>[!question]- 11. Типы связей между таблицами в БД. Привести примеры структур.
>
>**Определение**
>	
...
> [!question]- 12. ER-модель. Основные элементы и правила построения. Чтение готовой ER-модели.
>
>**Определение**
>	
...
> [!question]- 13. Создание таблицы. Синтаксис. Обязательные и опциональные ключевые слова. Объявление внешних ключей в таблице. Привести пример.
>**Синтаксис**
>```sql
...
> [!question]- 14. Изменение структуры таблицы. Особенности реализации стандарта SQL в SQLite. Привести несколько примеров.
...
> [!question]- 15. Добавление и удаление данных в таблице (в том числе разобрать случай с наличием связей в таблице). Возможные нарушения целостности структуры БД при модифицирующих операциях. При- вести несколько примеров.
>
...
>>VALUES (1, 'Иван Иванов', 101);
>>```
>
>>**Возможные проблемы**
>> - **Нарушение уникальности**: Если поле `id` является первичным ключом (PRIMARY KEY), попытка добавить строку с уже существующим значением `id` вызовет ошибку.<br><br>
>> - **Нарушение внешнего ключа**: Если `department_id` является внешним ключом (FOREIGN KEY), ссылающимся на таблицу `departments`, то значение `101` должно существовать в таблице `departments`. Иначе возникнет ошибка.
>
>
...
>
>>**Возможные проблемы**
>> - **Нарушение ссылочной целостности**: Если на запись с `id = 1` ссылаются другие таблицы через внешние ключи, удаление вызовет ошибку. Например, если есть таблица `salaries`, где `employee_id` ссылается на `employees.id`, то удаление сотрудника нарушит целостность.<br><br>
>> - **Каскадное удаление**: Если внешний ключ настроен с опцией `ON DELETE CASCADE`, то при удалении записи из родительской таблицы автоматически удалятся связанные записи в дочерних таблицах.
>
...
>
>**Возможные проблемы**
>> - **Нарушение внешнего ключа**: Если `department_id` является внешним ключом, новое значение `102` должно существовать в таблице `departments`. Иначе возникнет ошибка.<br><br>
>> - **Потеря данных**: Если обновление выполняется без условия `WHERE`, то изменятся все строки в таблице, что может привести к неожиданным последствиям.
>
>
 **Нарушения целостности структуры БД**
>>1. **Нарушение уникальности**
>
>>Возникает, если в таблицу добавляется дубликат значения в поле, которое должно быть уникальным (например, PRIMARY KEY или UNIQUE).
>>```sql
>>INSERT INTO employees (id, name) VALUES (1, 'Иван Иванов');
>>INSERT INTO employees (id, name) VALUES (1, 'Петр Петров'); -- Ошибка: дубликат id
>>```
>
>>2. **Нарушение ссылочной целостности**
>
>>Возникает, если нарушается связь между таблицами через внешние ключи.
>>```sql
>>DELETE FROM departments WHERE id = 101;
>>-- Ошибка: если в таблице employees есть записи с department_id = 101
>>```
>
>>3. **Нарушение ограничений (CHECK)**
>
>>Возникает, если данные не соответствуют заданным ограничениям.
>>```sql
>>ALTER TABLE employees ADD CONSTRAINT age_check CHECK (age >= 18);
>>INSERT INTO employees (id, name, age) VALUES (2, 'Анна', 16); -- Ошибка: возраст меньше 18
>>```
>
>>4. **Нарушение уникальности**
>
>>Возникает при неправильном использовании `UPDATE` или `DELETE` без условия `WHERE`.
>>```sql
>>DELETE FROM employees; -- Удалит все записи из таблицы
>>```
>

> [!question]- 16. Ограничения целостности при создании таблицы. Привести основные типы и несколько осмысленных примеров.
>
>> **Ограничения целостности** (constraints) в SQL используются для обеспечения корректности данных в таблицах. Они задаются при создании таблицы (или изменении существующей) и помогают предотвратить ввод некорректных данных. Рассмотрим основные типы ограничений и примеры их использования
>
...
>>>- В таблице может быть только один первичный ключ.(например, «Сотрудник»).
>>2. **FOREIGN KEY**
>>>- Обеспечивает ссылочную целостность между таблицами.
>>>- Значение должно существовать в связанной таблице (или быть `NULL`, если разрешено).
>>3. **UNIQUE**
>>>- Гарантирует, что все значения в столбце (или группе столбцов) уникальны.
>>>- В отличие от `PRIMARY KEY`, может быть несколько `UNIQUE` ограничений, и они могут содержать `NULL`.
>>4. **NOT NULL**
>>>- Запрещает вставку `NULL` в столбец.
>>5. **CHECK**
>>>- Проверяет, что значения в столбце удовлетворяют заданному условию.
>>1. **DEFAULT**
...
>
>1. Таблица `employees` с `PRIMARY KEY`, `NOT NULL` и `CHECK`
>>```sql
...
>
>
>2. Таблица `employees` с `FOREIGN KEY`
>>```sql
>>CREATE TABLE employees (
>>   id INT PRIMARY KEY,
>>   name VARCHAR(100) NOT NULL,
...
>
>
>3. Таблица `orders` с `CHECK` и `DEFAULT`
>>```sql
>>CREATE TABLE orders (
>>    id INT PRIMARY KEY, 
...
>>```sql
>>INSERT INTO employees (id, name) VALUES (1, 'Иван Иванов');
>>INSERT INTO employees (id, name) VALUES (1, 'Петр Петров');
>
>2. **Нарушение `FOREIGN KEY`**:
>>```sql
>>INSERT INTO employees (id, name, department_id) VALUES (2, 'Анна', 999);
>
>3. **Нарушение `UNIQUE`**:
>>```sql
>>INSERT INTO employees (id, name, email) VALUES (3, 'Мария', 'ivan@example.com');
>>INSERT INTO employees (id, name, email) VALUES (4, 'Ольга', 'ivan@example.com');
>>```
>
>4. **Нарушение `NOT NULL`**:
>>```sql
>>INSERT INTO employees (id, name) VALUES (5, NULL);
>>```
>
>5. **Нарушение `CHECK`**:
>>```sql
...
>[!question]- 17. Реализация различных типов связей средствами SQL. Приести пример структуры БД не менее чем из 4 таблиц с различными типами связей.
>**Теория**
...
> [!question]- 19. Оконные функции. Общий синтаксис, основные функции. Отличия оконных функций от аггрегирующих.
>**Определение**
...
> [!question]- 20. Различные виды упорядочивания в оконных функциях. Привести несколько примеров.
> **Теория**
>>	ASC
...
>>Возвращает нижний топ зарплаты
 ...
 > [!question]- 21. Основные правила работы с sqlite3. Создание БД и подключение к уже существующей БД. Описание основных объектов при работе с БД (подключение, курсор).
 >**Синтаксис и теория**
...
>[!question]- 22. Правила работы при запросе и изменении данных в БД с помощью sqlite3.
>>**Синтаксис и теория**
>
...
>[!question]- 23. Плейсхолдеры. Назвать минимум 2 причины их применения вместо форматных строк.
>**Теория и синтаксис**
...
>[!question]- 24. Object-relational mapping. Что такое и зачем применяется. Плюсы и минусы по сравнению с использованием SQL-скриптов.
>**Определение**
...
>`back_populates` - явное указывание на связи между таблицами. В отличие от `backref` нужно прописывать в каждой модели связь
>
> ```python
...
>
>** Принцип работы (это вроде не надо)**
>
